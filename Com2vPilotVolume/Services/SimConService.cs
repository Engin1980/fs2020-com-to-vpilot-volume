...
namespace eng.com2vPilotVolume.Services
{
  public class SimConService : BaseService
  {
    ...
    private class SimVar<T>(string name)
    {
    ...
    private static readonly RequestId REQUEST_EMPTY = new(-1);
    private static readonly TypeId TYPE_EMPTY = new(-1);
    ...
    private readonly AppSimConConfig settings;
    private readonly TaskCompletionSource<bool> stopTcs = new();
    ...
    public SimConService(AppSimConConfig settings)
    {
    ...
    {
      await Task.Run(() => this.connectionTimer.Enabled = true);
    }
